...
```
#Introduction 
For the semester long project I while we attempting to predict 
...
ether_df %>% summarise(mean_price = mean(price), median_price = median(price), sd_price = sd(price), iqr_price = IQR(price), n = n(), mad_price = mad(price), min_price = min(price), max_price = max(price))
