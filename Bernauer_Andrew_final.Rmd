...
library(keras)
set.seed(385)
 intrain <- createDataPartition(ether_df$price, p=0.75, list=FALSE)
 keras_train <- ether_df[intrain,][, c(-5, -6, -7, -9)]
 keras_testing <- ether_df[-intrain,][, c(-5, -6, -7, -9)]
 prepros_keras <- preProcess(keras_train, method = c("center", "scale"))
 TransformedTrain <- predict(prepros_keras, keras_train)
 TransformedTest <- predict(prepros_keras, keras_testing)
build_model <- function(){
 model <- keras_model_sequential() %>%
    layer_dense(units = 64, activation="relu",
                input_shape = dim(TransformedTrain)[2]) %>%
     layer_dense(units = 64, activation = "relu") %>%
     layer_dense(units = 1)
 model %>% compile(
   loss = "mse",
