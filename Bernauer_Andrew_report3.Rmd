...
covariates_train <- model.matrix(price~. , data = train_df)[, -1]

covariates_test <- model.matrix(price~. , data=test_df)[, -1]
response_train <- train_df[1]
response_test <- test_df[1]

grid <- 10^seq(10, -2, length.out = 100)

ridge_model <- glmnet(covariates_train, response_train, alpha = 0, lambda = grid, family ="mgaussian")

plot.glmnet(ridge_model, xvar = "lambda")

lasso_model <- glmnet(covariates_test, response_train, alpha = 1, lambda = grid, family = "mgaussian")
